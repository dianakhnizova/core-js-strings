...
function extractNameFromTemplate(value) {
  const match = value.match(/^Hello,\s([A-Za-z]+)\s([A-Za-z]+)!$/);
  if (match) {
...
function extractEmails(str) {
  ...
  const emails = str.replace(emailRegex, (match) => match);
  return emails ? emails.split(';').filter((email) => email.trim() !== '') : [];
}
